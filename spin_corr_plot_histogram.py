...
def load_histograms(pkl_path):
    ...
    ----------
    pkl_path : str
        Path to pickle file created by save_histograms().

    Returns
    -------
    tuple (dict, dict)
        (h_ref_dict, h_eft_dict)
    """
    ...
    return data["ref"], data["eft"]

from uncertainties import unumpy as unp

def safe_divide(num, den):
    """Safe element-wise divide for unumpy arrays."""
    num_nom, num_std = unp.nominal_values(num), unp.std_devs(num)
    den_nom, den_std = unp.nominal_values(den), unp.std_devs(den)

    # Avoid division by zero
    mask = den_nom != 0
    ratio_nom = np.divide(num_nom, den_nom, out=np.zeros_like(num_nom), where=mask)

    # Propagate errors manually: σ = sqrt( (σ_num/den)^2 + (num*σ_den/den^2)^2 )
    ratio_std = np.zeros_like(num_nom)
    ratio_std[mask] = np.sqrt(
        (num_std[mask] / den_nom[mask])**2 +
        ((num_nom[mask] * den_std[mask]) / (den_nom[mask]**2))**2
    )

    return unp.uarray(ratio_nom, ratio_std)
...
def values_and_errors(uarr):
    """Return nominal values and standard deviations from a uarray."""
    return unp.nominal_values(uarr), unp.std_devs(uarr)

def make_hist_from_uarray(y_uarr, bins):
    """Convert a unumpy array into a hist.Hist with values + variances."""
    h = hist.Hist(bins, storage=hist.storage.Weight())
    h.values()[...] = unp.nominal_values(y_uarr)
    h.variances()[...] = unp.std_devs(y_uarr) ** 2
    return h


def plot_histograms(h_ref_dict, h_eft_dict, outdir):
    os.makedirs(outdir, exist_ok=True)
    ...
    for xlabel, key, bkey in labels + composite_labels:
        ...
        # Ratio
        ratio = safe_divide(y_eft_norm, y_ref_norm)

        # Wrap back into hist.Hist
        h_eft_norm = make_hist_from_uarray(y_eft_norm, bins)
        h_ref_norm = make_hist_from_uarray(y_ref_norm, bins)
        h_ratio = make_hist_from_uarray(ratio, bins)
        ...
        mplhep.histplot(h_ref_norm, ax=ax_main, label="SM POWHEGv2", histtype="step")
        mplhep.histplot(h_eft_norm, ax=ax_main, label="SMEFTsim_noEFT", histtype="step")
        ax_main.set_ylabel("Normalized Diff. Xsec", fontsize=12)
        ax_main.legend()

        mplhep.histplot(h_ratio, ax=ax_ratio, histtype="errorbar", label="EFT/SM")
        ...
        ax_ratio.set_ylim(0.92, 1.08)

        fig.savefig(f"{outdir}/{key}.png", dpi=150, bbox_inches="tight")
        plt.close(fig)


def plot_histograms_multi(h_ref_dict, h_eft_dicts, eft_labels, outdir):
    os.makedirs(outdir, exist_ok=True)
    ...
    for xlabel, key, bkey in labels + composite_labels:
        ...
        y_ref = unp.uarray(h_ref.values(), np.sqrt(h_ref.variances()))
        if unp.nominal_values(y_ref).sum() == 0:
            continue
        ...
        # Reference
        mplhep.histplot(h_ref_norm, ax=ax_main, label="SM POWHEGv2", histtype="step", color="black")

        # EFTs
        colors = ["#0072B2", "#E69F00", "#009E73"]
        for eft_dict, eft_label, color in zip(h_eft_dicts, eft_labels, colors):
            h_eft = eft_dict.get(key)
            if h_eft is None:
                continue

            y_eft = unp.uarray(h_eft.values(), np.sqrt(h_eft.variances()))
            if unp.nominal_values(y_eft).sum() == 0:
                continue
            ...
            # Main plot
            mplhep.histplot(h_eft_norm, ax=ax_main, label=eft_label, histtype="step", color=color)

            # Ratio plot
            ratio = safe_divide(y_eft_norm, y_ref_norm)
            h_ratio = make_hist_from_uarray(ratio, bins)
            mplhep.histplot(h_ratio, ax=ax_ratio, histtype="step", label=eft_label, color=color)
        ...
        fig.savefig(f"{outdir}/{key}.png", dpi=150, bbox_inches="tight", transparent=True)
        plt.close(fig)


def main(): 
    # --- Directories ---
    outdir = "gen_spin_corr_plots_noEFT/comparison_plots/"
    os.makedirs(outdir, exist_ok=True)

    # Register awkward-vector behavior
    vector.register_awkward()

    # === Load histograms (one pickle per production) ===
    h_ref_dict, _ = load_histograms("gen_spin_corr_plots_noEFT/minimal_cut/histograms/histograms.pkl")
    _, h_eft_minimal = load_histograms("gen_spin_corr_plots_noEFT/minimal_cut/histograms/histograms.pkl")
    _, h_eft_alex = load_histograms("gen_spin_corr_plots_noEFT/Alex_noSMEFTsim/histograms/histograms.pkl")

    # === Plot SM vs EFTs ===
    plot_histograms_multi(
        h_ref_dict,
        [h_eft_alex, h_eft_minimal],
        ["EFT Alex_noSMEFTsim", "EFT minimal_cut"],
        outdir
    )
