...
# 
# echo '[RUN local] dim6top_ctG_m4_madspin'
# bash gridpack_generation_dim6top.sh ttbbllnunu_dim6top_ctG_p2 addons/cards/dim6top/ttbbllnunu_dim6top_ctG_p2 local ALL "$SCRAM_ARCH" "$CMSSW_VERSION"
...
make_gridpack () {
    ...
    echo "madgraph_version: MG5_aMC_v${MG_VERSION:-2.6.5}"
    ...
    # Lingqiang patch: use MG5_aMC v2.6.5 instead of the legacy v2.6.5.
    # The extracted directory name is converted from MG5_aMC_v2.6.5 -> MG5_aMC_v3_4_0
    # to match the naming style used elsewhere in this CMS gridpack wrapper.
    MG_VERSION="2.6.5"
    MG=MG5_aMC_v${MG_VERSION}${MG_EXT}
    ...
    if [ ! -d ${GEN_FOLDER}/${name}_gridpack ]; then
      ...
      #############################################
      if [ -f "${PRODHOME}/${MG}" ]; then
        echo "Using local MadGraph tarball: ${PRODHOME}/${MG}"
        cp "${PRODHOME}/${MG}" .
      else
        echo "Downloading MadGraph tarball: ${MGSOURCE}"
      ...
      #############################################
      # Always refresh dim6top_LO_UFO from the official CMS generator area
      # This avoids accidentally using a locally modified/polluted UFO checkout.
      #############################################
      DIM6TOP_UFO_TARBALL="dim6top_LO_UFO.tar.gz"
      DIM6TOP_UFO_URL="https://cms-project-generators.web.cern.ch/cms-project-generators/${DIM6TOP_UFO_TARBALL}"

      echo "[INFO] Refreshing official dim6top_LO_UFO from ${DIM6TOP_UFO_URL}"
      mkdir -p "$PRODHOME/addons/models"
      rm -rf "$PRODHOME/addons/models/dim6top_LO_UFO"

      if [ -f "$PRODHOME/${DIM6TOP_UFO_TARBALL}" ]; then
          echo "[INFO] Using local cached $PRODHOME/${DIM6TOP_UFO_TARBALL}"
          cp "$PRODHOME/${DIM6TOP_UFO_TARBALL}" .
      else
          wget --no-check-certificate "$DIM6TOP_UFO_URL"
      fi

      tar xzf "${DIM6TOP_UFO_TARBALL}" -C "$PRODHOME/addons/models"
      rm -f "${DIM6TOP_UFO_TARBALL}"

      if [ ! -d "$PRODHOME/addons/models/dim6top_LO_UFO" ]; then
          echo "[ERROR] Expected $PRODHOME/addons/models/dim6top_LO_UFO after unpacking ${DIM6TOP_UFO_TARBALL}"
          echo "[ERROR] Check the tarball structure from ${DIM6TOP_UFO_URL}"
          if [ "${BASH_SOURCE[0]}" != "${0}" ]; then return 1; else exit 1; fi
      fi

      rm -rf "$MGBASEDIRORIG/models/dim6top_LO_UFO"
      cp -a "$PRODHOME/addons/models/dim6top_LO_UFO" "$MGBASEDIRORIG/models/"
