...
# python3 Make_SlurmJobs_nanogen_from_gridpacks_total_per_gridpack.py   --gridpack-glob '/depot/cms/top/he614/cmseft/generation/genproductions/bin/MadGraph5_aMCatNLO/TT01j2lBSMRef_*_slc7_amd64_gcc700_CMSSW_10_6_19_tarball.tar.xz'   --total-events-per-gridpack 500000   --events-per-job 5000 --debug
...
DEFAULT_JOB_DIR = DEFAULT_BASE / 'slurm_nanogen_gridpacks'
DEFAULT_OUT_DIR = DEFAULT_BASE / 'nanogen_outputs_gridpacks'
...
def main():
    ...
    parser.add_argument('--gridpack-glob', required=True, help='Glob matching the gridpack tarballs')
    ...
    gridpacks = sorted(Path(p).resolve() for p in glob.glob(args.gridpack_glob))
    if not gridpacks:
        raise RuntimeError('No gridpacks matched: {0}'.format(args.gridpack_glob))
    ...
    print('[OK] Total SLURM jobs: {0}'.format(n_jobs))
